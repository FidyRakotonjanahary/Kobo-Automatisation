...
from app.core.exceptions import GoogleAuthError, GoogleQuotaError, GooglePermissionError
...
class GoogleService:
    ...
    def _handle_google_error(self, e: HttpError):
        ...
        if status == 401: raise GoogleAuthError(detail=reason)
        if status == 403:
        ...
        if status == 404: raise Exception("Dossier ou fichier Drive introuvable.")
        raise Exception(f"Erreur Google Drive ({status}): {reason}")

    def upload_file(self, local_path: str, folder_id: str, display_name: str, convert: bool = False) -> str:
        try:
            file_metadata = {"name": display_name, "parents": [folder_id]}
            if convert:
                file_metadata["mimeType"] = "application/vnd.google-apps.spreadsheet"
            media = MediaFileUpload(local_path, resumable=True)
            file = self.drive.files().create(body=file_metadata, media_body=media, fields="id,webViewLink", supportsAllDrives=True).execute()
            try:
                self.drive.permissions().create(fileId=file["id"], body={'type': 'anyone', 'role': 'writer'}).execute()
            except: pass
    ...
    def create_folder(self, name: str, parent_id: Optional[str] = None) -> str:
        try:
            ...
            file = self.drive.files().create(body=file_metadata, fields="id", supportsAllDrives=True).execute()
            return file.get("id")
        except HttpError as e:
            self._handle_google_error(e)
    ...
    def update_cell(self, spreadsheet_id: str, range_name: str, value: str):
        try:
            body = {"values": [[value]]}
            self.sheets.spreadsheets().values().update(spreadsheetId=spreadsheet_id, range=range_name, valueInputOption="RAW", body=body).execute()
        except HttpError as e:
            self._handle_google_error(e)
