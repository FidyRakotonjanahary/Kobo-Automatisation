...
class MediaEngine:
    ...
    async def pre_flight_check(self, spreadsheet_id: str, drive_folder_id: str):
        ...
        try:
            ...
            self.google.sheets.spreadsheets().get(spreadsheetId=spreadsheet_id, fields="id").execute()
            # Tester l'accès au dossier Drive
            self.google.drive.files().get(fileId=drive_folder_id, fields="id").execute()
        except Exception as e:
            logger.error(f"Pre-flight check failed: {e}")
            raise AppException("Impossible d'accéder au Google Sheet ou au dossier Drive. Vérifiez vos permissions.", 403)

    async def migrate_sheet(self, spreadsheet_id: str, sheet_name: Optional[str], drive_folder_id: str, on_progress: Optional[callable] = None):
        def report(msg):
            logger.info(msg)
        ...
        try:
            ...
            ss_metadata = self.google.sheets.spreadsheets().get(spreadsheetId=spreadsheet_id).execute()
            all_sheets = [s['properties']['title'] for s in ss_metadata.get('sheets', [])]
            
            if sheet_name and sheet_name.strip():
                if sheet_name in all_sheets: target_sheets = [sheet_name]
                else: raise AppException(f"L'onglet '{sheet_name}' n'existe pas.", 404)
            else:
        ...
        for s_name in target_sheets:
            ...
            sheet_stats = await self._migrate_single_tab(spreadsheet_id, s_name, drive_folder_id, on_progress)
            global_stats["success"] += sheet_stats["success"]
            global_stats["failed"] += sheet_stats["failed"]
        return global_stats

    async def _migrate_single_tab(self, spreadsheet_id: str, sheet_name: str, drive_folder_id: str, on_progress: Optional[callable] = None):
        def report(msg):
            logger.info(msg)
            if on_progress: on_progress(msg)
        ...
        if not rows: return {"success": 0, "failed": 0}
        ...
        for row_idx, row in enumerate(data_rows):
            ...
            for col_idx in url_cols:
                ...
                if col_idx > 0 and col_idx - 1 < len(row):
                    ...
                    if custom_name and custom_name != "None": display_name = f"{custom_name}.jpg"

                report(f"⬇️ En cours : Ligne {real_row_idx}...")
                local_path = os.path.join(self.temp_dir, f"temp_{real_row_idx}_{col_idx}.jpg")
                ...
                if download_success:
                    try:
                        drive_link = self.google.upload_file(local_path, drive_folder_id, display_name)
